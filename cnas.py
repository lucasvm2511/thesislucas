...
class CNAS:
    def __init__(self, kwargs):
        ...
        self.iterations = kwargs.pop('iterations', 2)  #30 number of iterations to run search
        self.n_doe = kwargs.pop('n_doe', 100)  # 100number of architectures to train before fit surrogate model
        self.n_iter = kwargs.pop('n_iter', 8)  #8 1number of architectures to train in each iteration
        self.first_predictor = kwargs.pop('first_predictor', 'mlp')  # surrogate 1st objective
        ...
        self.gate_training_epochs = kwargs.pop('gate_training_epochs', 0) # epochs to train gates (early exit or layer skipping)
    ...
    def _evaluate(self, archs, it): #Train and evaluate subnets and save them in folders
        ...
        if isinstance(self.search_space, NASBench201SearchSpace) and self.bench_eval:
            ...
            prepare_eval_folder(
                ...
                warmup_ee_epochs = self.warmup_ee_epochs, gate_training_epochs = self.gate_training_epochs, quantization=self.quantization, drift=self.drift, n_classes=self.n_classes)
            ...
            # Wait a bit to ensure all processes have fully released GPU memory
            import time
            print("Waiting for all training processes to complete and release GPU memory...")
            time.sleep(10)  # Give processes time to clean up
    ...
    def _nextSingleObj(self, archive, acc_predictor, K):
        ...
        # kick-off the search
        res = minimize(
            problem, method, ('n_gen', 60), save_history=True, verbose=False) #verbose=True displays some printouts
    ...
    def _nextMultiObj(self, archive, acc_predictor, compl_predictor, K):
        ...
        # kick-off the search with simple generation-based termination
        res = minimize(
            problem, method, ('n_gen', 20), save_history=True, verbose=False) #verbose=True displays some printouts #default 20 generations
...
if __name__ == '__main__':
    ...
    parser.add_argument('--iterations', type=int, default=2,#30
                        help='number of search iterations')
    parser.add_argument('--n_doe', type=int, default=100, #100
                        help='initial sample size for DOE')
    parser.add_argument('--n_iter', type=int, default=8, #8
                        help='number of architectures to high-fidelity eval (low level) in each iteration')
    parser.add_argument('--first_predictor', type=str, default='mlp', #rbf
    ...
    parser.add_argument('--gate_training_epochs', type = int, default=0, help='epochs to train gates (for early exit or layer skipping models)')
