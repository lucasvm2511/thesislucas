...
def bash_command_template_multi_exits(**kwargs):
    ...
    cfg['ee_epochs'] = kwargs['gate_training_epochs']
    cfg['n_workers'] = kwargs['n_workers']

    execution_line = "python3 ee_train.py".format(gpus)
    for k, v in cfg.items():
        if v is not None:
            if isinstance(v, bool):
                if v:
...
def bash_command_template_layer_skipping(**kwargs):
    ...
    cfg['val_split'] = kwargs['val_split']
    cfg['backbone_epochs'] = kwargs['n_epochs']
    cfg['training_epochs'] = kwargs.get('gate_training_epochs', 10)  # Use gate_training_epochs for layer skipping
    cfg['n_workers'] = kwargs['n_workers']

    execution_line = "python3 ls_train.py".format(gpus)
    for k, v in cfg.items():
        if v is not None:
            if isinstance(v, bool):
                if v:
...
def bash_command_template_single_exit(**kwargs):
    ...
    execution_line = "python3 train.py".format(gpus)
    for k, v in cfg.items():
        if v is not None:
            if isinstance(v, bool):
                if v:
...
def get_template_by_type(gpus, subnet, save, type, **kwargs):
    ...
    elif type == 'layer_skipping':
        return bash_command_template_layer_skipping(gpus=gpus, subnet=subnet, save=save, **kwargs)
    else:
