...
class MLP:
    ...
    def get_feature_importance(self, x_data, method='gradient', device='cpu'):
        """
        Calculate feature importance for MLP using gradient-based methods.
        
        Args:
            x_data: Input data (numpy array)
            method: 'gradient' for gradient-based importance, 'weight' for first-layer weights
        ...
        if method == 'gradient':
            # Gradient-based importance: average gradient magnitude across samples
            ...
            # Backward pass: gradient of output w.r.t. inputs
            gradients = torch.autograd.grad(
                outputs.sum(), inputs, create_graph=False
            )[0]
            
            # Average absolute gradient across samples
            importance = gradients.abs().mean(dim=0).cpu().detach().numpy()
            
        elif method == 'weight':
            # First-layer weight magnitude as importance
            first_layer = self.model.stem[0]  # First linear layer
        ...
        return {'importance': importance, 'method': method}
    
    def print_feature_importance(self, x_data, feature_names=None, top_k=10, method='gradient', device='cpu'):
        """
        Print feature importance analysis for MLP.
        
        Args:
            x_data: Training data for gradient calculation
            feature_names: Optional list of feature names
            top_k: Number of top features to display
            method: 'gradient' or 'weight'
            device: Device to run on
        """
        importance_data = self.get_feature_importance(x_data, method=method, device=device)
        ...
        print("\n" + "="*80)
        print(f"MLP SURROGATE MODEL - FEATURE IMPORTANCE ANALYSIS ({method.upper()} method)")
        print("="*80)
        ...
        for rank, idx in enumerate(sorted_indices[:top_k], 1):
            ...
            print(f"{rank:<6} {feat_name:<30} {importance[idx]:<12.4f} {pct:<12.2f}")
        
        print("-"*80)
        print(f"Total importance (top {top_k}): {importance[sorted_indices[:top_k]].sum():.4f} ({importance[sorted_indices[:top_k]].sum()*100:.1f}%)")
        print("="*80 + "\n")
